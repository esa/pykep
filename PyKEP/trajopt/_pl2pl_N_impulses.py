...
class pl2pl_N_impulses(base_problem):
	...
	The decision vector is [t0,T] + [alpha,u,v,V_inf]*(N-2) +[alpha] + ([tf])..... in the units: [mjd2000,days] + [nd,nd,m/sec,nd] + [nd] + [mjd2000]

	Vinf = Vinf_mag*(cos(theta)*cos(phi)i+cos(theta)*sin(phi)j+sin(phi)k) and theta = 2*pi*u, phi = acos(2*v-1)-pi/2
	...
	def __init__(self, 
		...
		* multi_objective: 	when True, a multi-objective problem is constructed with DV and time of flight as objectives
		...
		if (t0 == None and not phase_free):
			t0 = [epoch(0),epoch(1000)]
		if (t0 != None and phase_free):
	...
	def plot(self,x):
		...
		for i,time in enumerate(T[:-1]):	
			...
			# We apply the (i+1)-th impulse
			vsc 	= [a+b for a,b in zip(vsc,[Vinfx,Vinfy,Vinfz])]
			plot_kepler(rsc,vsc,T[i]*DAY2SEC,self.__common_mu,N = 200, color='b', legend=False, units = AU, ax=axis)
			rsc,vsc = propagate_lagrangian(rsc,vsc,T[i]*DAY2SEC,self.__common_mu)
		...
		dt = T[-1]*DAY2SEC
		l = lambert_problem(rsc,r_target,dt,self.__common_mu, cw, False)
		plot_lambert(l, sol = 0, color='r', legend=False, units = AU, ax=axis,N=200)
		plt.show()
		return axis