...
std::tuple<std::array<double, 49>, std::array<double, 49>, std::vector<double>, std::vector<double>> zoh::compute_mc_grad() const
{
	using xt::adapt;
	using xt::view;
	using xt::all;
	...
	if (m_nseg_fwd > 0) {
		...
		xt::view(dmcdtgrid_fwd, xt::all(), m_nseg_fwd, xt::all()) = xt::eval(_dot<7,7,1>(M_fwd.back(), dyn_fwd.back()));
	...
	// 4. dmc/dx1
	for (unsigned i = 0; i < 49; ++i) dmc_dx1[i] = -M_bck[0].data()[i];

	// 5. dmc/dcontrols (backward)
	// Fill C_bck from right to left so that the first backward segment's gradient appears in the last block (Python-style)
	...
	if (m_nseg_bck > 0) {
		...
		xt::view(dmcdtgrid_bck, xt::all(), m_nseg_fwd, xt::all()) -= xt::eval(_dot<7,7,1>(M_bck.back(), dyn_bck.back()));
	...
	if (m_nseg_fwd > 0) xt::view(x_dmc_dcontrols, xt::all(), xt::range(0, 4 * m_nseg_fwd)) = C_fwd;
	if (m_nseg_bck > 0) xt::view(x_dmc_dcontrols, xt::all(), xt::range(4 * m_nseg_fwd, 4 * m_nseg)) = -C_bck;
	...
	// flatten last dimension for output
	x_dmc_dtgrid = xt::reshape_view(x_dmc_dtgrid, {7u, m_nseg + 1});

	// Flatten dmc_dcontrols and dmc_dtgrid for output
...
void zoh::sanity_checks() const
{
	...
	if (m_ta.get_dim() != 7u) {
		throw std::logic_error(fmt::format(
			"Attempting to construct a zoh leg with a Taylor adaptive integrator state dimension of {}, while 7 is required.",
			m_ta.get_dim()));
	}

	if (m_ta.get_pars().size() <= 4u) {
		throw std::logic_error(fmt::format(
			"Attempting to construct a zoh leg with a Taylor adaptive integrator parameters dimension of {}, while >4 is required.",
			m_ta.get_pars().size()));
	}
	...
	if (m_ta_var) {
		...
		if (m_ta_var->get_dim() != 84u) {
			throw std::logic_error(fmt::format(
				"Attempting to construct a zoh leg with a variational Taylor adaptive integrator state dimension of {}, while 84 is required.",
				m_ta_var->get_dim()));
		}
		if (m_ta_var->get_pars().size() != m_ta.get_pars().size()) {
			throw std::logic_error(
				"The variational and nominal Taylor adaptive integrators for a zoh leg must expose the same number of parameters.");
		}
	}
}
