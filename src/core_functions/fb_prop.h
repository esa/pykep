...
/**
 * This function can be used to propagate a given fly-by hyperbola from its input conditions (entrance in the sphere of
 * influence)
...
inline void fb_prop(array3D &v_out, const array3D &v_in, const array3D &v_pla, const double &rp, const double &beta,
                    const double &mu)
{
    ...
    v_out[0] = v_pla[0] + v_rel_in_norm * cos(delta) * i_hat[0] + v_rel_in_norm * cos(beta) * sin(delta) * j_hat[0]
               + v_rel_in_norm * sin(beta) * sin(delta) * k_hat[0];
    v_out[1] = v_pla[1] + v_rel_in_norm * cos(delta) * i_hat[1] + v_rel_in_norm * cos(beta) * sin(delta) * j_hat[1]
               + v_rel_in_norm * sin(beta) * sin(delta) * k_hat[1];
    v_out[2] = v_pla[2] + v_rel_in_norm * cos(delta) * i_hat[2] + v_rel_in_norm * cos(beta) * sin(delta) * j_hat[2]
               + v_rel_in_norm * sin(beta) * sin(delta) * k_hat[2];
    return;
}
} // namespace end
