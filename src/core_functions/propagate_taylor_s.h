...
template <class T>
double
propagate_taylor_s_step(T &r0, T &v0, double &m0, double &t0, const double &sf,
                        const int &order, const T &thrust, const double &mu,
                        const double &sundmann_alpha, const double &sundmann_c,
                        const double &veff, const double &xm,
                        const double &eps_a, const double &eps_r,
                        std::vector<std::array<double, 8>> &x,
                        std::vector<std::array<double, 25>> &u) {

  double sqrtT = sqrt(thrust[0] * thrust[0] + thrust[1] * thrust[1] +
                      thrust[2] * thrust[2]);
  ...
  while (n < order) {
    ...
    for (int j = 0; j <= n; j++)
      u[n][8] += u[j][0] * u[n - j][0]; // x^2
    for (int j = 0; j <= n; j++)
      u[n][9] += u[j][1] * u[n - j][1]; // y^2
    for (int j = 0; j <= n; j++)
      u[n][10] += u[j][2] * u[n - j][2];     // z^2
    ...
    if (n == 0) {
      ...
      for (int j = 0; j < n; ++j)
        u[n][12] += (gamma * n - j * (gamma + 1)) * u[n - j][11] * u[j][12];
    ...
    if (n == 0) {
      ...
      for (int j = 0; j < n; ++j)
        u[n][13] += (sigma * n - j * (sigma + 1)) * u[n - j][11] * u[j][13];
      u[n][13] = u[n][13] / n / u[0][11];
    } // 1 / r^(3-alpha)

    for (int j = 0; j <= n; j++)
      u[n][14] += u[j][13] * u[n - j][0]; // 1 / r^(3-alpha) x
    for (int j = 0; j <= n; j++)
      u[n][15] += u[j][13] * u[n - j][1]; // 1 / r^(3-alpha) y
    for (int j = 0; j <= n; j++)
      u[n][16] += u[j][13] * u[n - j][2]; // 1 / r^(3-alpha) z

    if (n == 0) {
      u[n][17] = u[n][12] / u[n][6];
    } else {
      for (int j = 1; j <= n; j++)
        u[n][17] += u[j][6] * u[n - j][17];
      u[n][17] = 1.0 / u[0][6] * (u[n][12] - u[n][17]);
    } // r^alpha/m

    for (int j = 0; j <= n; j++)
      u[n][18] += u[j][3] * u[n - j][12]; // eq1
    for (int j = 0; j <= n; j++)
      u[n][19] += u[j][4] * u[n - j][12]; // eq2
    for (int j = 0; j <= n; j++)
      u[n][20] += u[j][5] * u[n - j][12];             // eq3
...
/// Taylor series propagation of a constant thrust arc using the Generalized
/// Sundmann Transformation
/**
 * This template function propagates an initial state using the generalized
 * Sundmann-Transformation. The independent variable thus is not
 * time, but \f$ ds = c r^\alpha dt \f$
 *
 * \param[in,out]	r0 initial position vector. On output contains the
 * propagated position. (r0[1],r0[2],r0[3] need to be preallocated, suggested
 * template type is std::array<double>,3))
 * \param[in,out]	v0 initial velocity vector. On output contains the
 ...
 * \param[in,out]	sf propagation pesudo-time (can be negative). If the
 * maximum number of iterations is reached, the pseudo-time is returned where
 * the state is calculated for the last time
 * \param[in]		mu central body gravitational parameter
 * \param[in]		veff the product g0*Isp characteristic of the engine
 * \param[in]		c constant in the generalized Sundmann transform defaults
 * to 1.0
 * \param[in]		alpha exponent in the generalized Sundmann transform
 * (defaults to 1.5)
 * \param[in]		log10tolerance logarithm of the desired absolute
 * tolerance
 * \param[in]		log10rtolerance logarithm of the desired relative
 * tolerance
 ...
 */
template <class T>
void propagate_taylor_s(T &r0, T &v0, double &m0, double &t0, const T &thrust,
                        const double &sf, const double &mu = 1,
                        const double &veff = 1, const double &c = 1.0,
                        const double &alpha = 1.5,
                        const int &log10tolerance = -10,
                        const int &log10rtolerance = -10,
                        const int &max_iter = 10000,
                        const int &max_order = 3000) {

  std::array<double, 8> dumb;
  std::array<double, 25> dumb2;
  for (int i = 0; i < 8; ++i)
    dumb[i] = 0;
  for (int i = 0; i < 25; ++i)
    dumb2[i] = 0;

  std::vector<std::array<double, 8>> _x;  // x[order][var]
  std::vector<std::array<double, 25>> _u; // u[order][var]
  ...
  for (j = 0; j < max_iter; ++j) {
    ...
    if (order > max_order)
      throw_value_error("Polynomial order is too high.....");

    // 3 - We allocate or deallocate memory if necessary
    _x.assign(order + 1, dumb);
    _u.assign(order, dumb2);
    double h =
        propagate_taylor_s_step(r0, v0, m0, t0, step, order, thrust, mu, alpha,
                                c, veff, xm, eps_a, eps_r, _x, _u);
  ...
  if (j > max_iter - 1)
    throw_value_error(
        "Maximum number of iteration reached in Taylor integration (sundmann)");
}
