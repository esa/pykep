 ...
 * and inclination. The argument of perigee is not accounted for and thus the approximation is breaking down at high eccentircities
 *
...
inline double three_impulses_approx(double a1, double e1, double i1, double W1, double a2, double e2, double i2, double W2, double mu)
{
 ...
 * @author Dario Izzo (dario.izzo _AT_ googlemail.com)
 */
inline double three_impulses_approx(const planet &pl1, const planet &pl2)
{
    ...
    array6D el1 = pl1.get_elements();
    array6D el2 = pl2.get_elements();
    return three_impulses_approx(el1[0], el1[1], el1[2], el1[3], el2[0], el2[1], el2[2], el2[3], pl1.get_mu_central_body());
}
 ...
 * and inclination. The argument of perigee is not accounted for and thus the approximation is breaking down at high eccentircities
 *
 * This overload allows to compute the orbital parameters at a given epoch, useful for non keplerian ephemerides as the ones of
 * kep_toolbox::planet_tle
 *
 * \param[in] pl1 departure kep_toolbox::planet
 * \param[in] pl2 target kep_toolbox::planet
 * \param[in] pl1 departure kep_toolbox::epoch (note that this is only used to compute the orbital parameters of the departure orbit)
 ...
 * @author Dario Izzo (dario.izzo _AT_ googlemail.com)
 */
inline double three_impulses_approx(const planet &pl1, const planet &pl2, const epoch &ep1, const epoch &ep2)
{
	if (pl1.get_mu_central_body() != pl2.get_mu_central_body()) {
		throw_value_error("The departure and arrival planets do not have the same central body gravitational parameter");
	}
    array6D el1 = pl1.get_elements(ep1);
    array6D el2 = pl2.get_elements(ep2);
    return three_impulses_approx(el1[0], el1[1], el1[2], el1[3], el2[0], el2[1], el2[2], el2[3], pl1.get_mu_central_body());
}
