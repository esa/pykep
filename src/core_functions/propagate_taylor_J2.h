...
template <class T>
double propagate_taylor_J2_step(T &r0, T &v0, double &m0, const double &h,
                                const int &order, const T &thrust,
                                const double &mu, const double &veff,
                                const double &J2RG2, const double &xm,
                                const double &eps_a, const double &eps_r,
                                std::vector<std::array<double, 7>> &x,
  ...
  while (n < order) {
    ...
    for (int j = 0; j <= n; j++)
      u[n][23] +=
          u[j][20] * u[n - j][22]; // 3/2 J2 RG^2 / r^2 * (1 - 5 z^2/r^2)
    u[n][24] = 3 * u[n][18] - 5 * u[n][21]; // 3 - 5 z^2/r^2
    for (int j = 0; j <= n; j++)
      u[n][25] +=
          u[j][20] * u[n - j][24];  // 3/2 J2 RG^2 / r^2 * (3 - 5 z^2/r^2)
    ...
    for (int j = 0; j <= n; j++)
      u[n][28] +=
          u[j][14] *
          u[n - j]
           [26]; // -mu/r^3 * x * (1 + 3/2 J2 RG^2 / r^2 * (1 - 5 z^2/r^2))
    for (int j = 0; j <= n; j++)
      u[n][29] +=
          u[j][15] *
          u[n - j]
           [26]; // -mu/r^3 * y * (1 + 3/2 J2 RG^2 / r^2 * (1 - 5 z^2/r^2))
    for (int j = 0; j <= n; j++)
      u[n][30] +=
          u[j][16] *
          u[n - j]
           [27]; // -mu/r^3 * z * (1 + 3/2 J2 RG^2 / r^2 * (3 - 5 z^2/r^2))
 ...
 */
template <class T>
void propagate_taylor_J2(T &r0, T &v0, double &m0, const T &u, const double &t0,
                         const double &mu = 1, const double &veff = 1,
                         const double &J2RG2 = 0.,
  ...
  for (j = 0; j < max_iter; ++j) {
    ...
    double h = propagate_taylor_J2_step(r0, v0, m0, step, order, u, mu, veff,
                                        J2RG2, xm, eps_a, eps_r, _x, _u);
  ...
  if (j > max_iter - 1)
    throw_value_error("Maximum number of iteration reached");
}
