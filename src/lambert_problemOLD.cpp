 ...
 */
lambert_problemOLD::lambert_problemOLD(const array3D &r1, const array3D &r2, const double &tof, const double &mu,
    ...
    : m_r1(r1), m_r2(r2), m_tof(tof), m_mu(mu), m_has_converged(true), m_multi_revs(multi_revs)
{
    ...
    for (int i = 0; i < m_Nmax; ++i) {
        m_iters[1 + 2 * i] = lambert_3d(m_v1[1 + 2 * i], m_v2[1 + 2 * i], m_a[1 + 2 * i], m_p[1 + 2 * i], r1, r2, tof,
                                        mu, m_lw, i + 1, 'l');
        m_iters[2 + 2 * i] = lambert_3d(m_v1[2 + 2 * i], m_v2[2 + 2 * i], m_a[2 + 2 * i], m_p[2 + 2 * i], r1, r2, tof,
                                        mu, m_lw, i + 1, 'r');
    }
...
std::ostream &operator<<(std::ostream &s, const lambert_problemOLD &lp)
{
    ...
    for (int i = 0; i < lp.m_Nmax; ++i) {
        s << i + 1 << " revs,  left. Iters: " << lp.m_iters[1 + 2 * i] << ", a: " << lp.m_a[1 + 2 * i]
          << ", p: " << lp.m_p[1 + 2 * i] << std::endl;
        s << "\tv1= " << lp.m_v1[1 + 2 * i] << " v2= " << lp.m_v2[1 + 2 * i] << std::endl;
        s << i + 1 << " revs, right. Iters: " << lp.m_iters[2 + 2 * i] << ", a: " << lp.m_a[2 + 2 * i]
          << ", p: " << lp.m_p[2 + 2 * i] << std::endl;
