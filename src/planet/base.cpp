...
double base::compute_period(const epoch& when) const {
	return 2* boost::math::constants::pi<double>() * std::sqrt(std::pow(compute_elements(when)[0],3) / get_mu_central_body());
}
 ...
 */
double base::get_mu_central_body() const {return m_mu_central_body;};
 ...
 */
double base::get_safe_radius() const {return m_safe_radius;};
