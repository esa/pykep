...
planet_ss::planet_ss(const std::string& name)
{
	...
	case (2): {
		...
		break;
	case (3): {
			double E[6] = {0.999988049532578E+00 * ASTRO_AU, 1.671681163160e-02, 0.8854353079654e-03 * ASTRO_DEG2RAD,  1.7540647696473E+03 * ASTRO_DEG2RAD,  2.8761577546182E+03* ASTRO_DEG2RAD, 2.5760683707535E+03 * ASTRO_DEG2RAD};
			std::copy(E, E + 6, keplerian_elements_.begin());
			mjd2000 = epoch(54000,epoch::MJD).mjd2000();
			radius_ = 6378000;
			safe_radius_ = radius_*1.1;
