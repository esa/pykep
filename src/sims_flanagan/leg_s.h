...
public:
  ...
  */
  leg_s()
      : m_ti(), m_xi(), m_throttles(), m_tf(), m_xf(), m_sf(0), m_sc(), m_mu(0),
        m_c(), m_alpha(), m_tol(-10), m_states(), m_ceq(), m_cineq(), m_dv() {}

  /// Constructor
  /**
   * Constructs an empty leg allocating memory for a given number of segments.
  */
  leg_s(const unsigned int &n_seg, const double &c, const double &alpha,
        const double &tol = -10)
      : m_ti(), m_xi(), m_throttles(n_seg), m_tf(), m_xf(), m_sf(0), m_sc(),
        m_mu(), m_c(c), m_alpha(alpha), m_tol(tol), m_states(n_seg + 2),
        m_ceq(), m_cineq(n_seg), m_dv(n_seg) {}
  ...
  */
  ...
  void set_leg(
      const epoch &epoch_i, const sc_state &state_i, it_type throttles_start,
      it_type throttles_end, const epoch &epoch_f, const sc_state &state_f,
      const double &sf, const spacecraft &sc_, const double &mu_,
      typename boost::enable_if<boost::is_same<
          typename std::iterator_traits<it_type>::value_type, double>>::type * =
          0) {
    // We check data consistency
    if (std::distance(throttles_start, throttles_end) % 3) {
      throw_value_error(
          "The length of the throttles list must be a multiple of 3");
    }
    if (std::distance(throttles_start, throttles_end) / 3 !=
        (int)m_throttles.size()) {
    ...
    if (mu_ <= 0) {
      throw_value_error(
          "Gravity parameter must be larger than zero (forgot to set it?)");
    }
    ...
    if (sc_.get_mass() == 0) {
      throw_value_error(
          "Spacecraft mass must be larger than zero (forgot to set it?)");
    }
    ...
    for (size_t i = 0; i < m_throttles.size(); ++i) {
      kep_toolbox::array3D tmp = {{*(throttles_start + 3 * i),
                                   *(throttles_start + 3 * i + 1),
                                   *(throttles_start + 3 * i + 2)}};
  ...
  void set_leg(const epoch &epoch_i, const sc_state &state_i,
               const std::vector<double> &thrott, const epoch &epoch_f,
               const sc_state &state_f, const double &sf) {
    set_leg(epoch_i, state_i, thrott.begin(), thrott.end(), epoch_f, state_f,
            sf, m_sc, m_mu);
  }
  ...
  void set_leg(const epoch &epoch_i, const sc_state &state_i,
               const std::vector<double> &thrott, const epoch &epoch_f,
               const sc_state &state_f, const double &sf, const spacecraft &sc_,
               const double &mu_) {
    set_leg(epoch_i, state_i, thrott.begin(), thrott.end(), epoch_f, state_f,
            sf, sc_, mu_);
  }
  ...
  */
  ...
  const std::array<double, 8> &compute_mismatch_con() const {
    ...
    double dt = (m_tf.mjd2000() - m_ti.mjd2000()) *
                ASTRO_DAY2SEC; // length of the leg in seconds
    ...
    for (int i = 0; i < n_seg_fwd; i++) {
      ...
      propagate_taylor_s(rfwd, vfwd, mfwd, tfwd, thrust, ds, m_mu, veff, m_c,
                         m_alpha, m_tol, m_tol);
    }
    ...
    for (int i = 0; i < n_seg_back; i++) {
      for (int j = 0; j < 3; j++) {
        thrust[j] =
            max_thrust * m_throttles[m_throttles.size() - i - 1].get_value()[j];
      }
      propagate_taylor_s(rback, vback, mback, tback, thrust, -ds, m_mu, veff,
                         m_c, m_alpha, m_tol, m_tol);
    }
  ...
  */
  const std::vector<std::array<double, 11>> &compute_states() const {
    ...
    for (int i = 0; i < n_seg_fwd; i++) {
      ...
      try {
        ...
        throw_value_error(
            "Could not compute the states ... check your data!!!");
      }
  ...
  const std::vector<throttle> &get_throttles() {
    ...
    for (size_t i = 0; i < n_seg_fwd; ++i) {
      m_throttles[i].set_start(
          epoch(states[i][0] * ASTRO_SEC2DAY + m_ti.mjd2000()));
      m_throttles[i].set_end(
          epoch(states[i + 1][0] * ASTRO_SEC2DAY + m_ti.mjd2000()));
    }
    ...
    for (size_t i = n_seg_fwd; i < n_seg; ++i) {
      m_throttles[i].set_start(
          epoch(states[i + 1][0] * ASTRO_SEC2DAY + m_ti.mjd2000()));
      m_throttles[i].set_end(
          epoch(states[i + 2][0] * ASTRO_SEC2DAY + m_ti.mjd2000()));
    }
    return m_throttles;
  }
  //@}
...
protected:
  void record_states(const double &t, const array3D &r, const array3D &v,
                     const double &m, const array3D &thrust,
