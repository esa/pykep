...
/*                example, when the SPICELIB progress reporting functions */
...
/*                ET is an epoch belonging to the interval [IVBEG, */
/*                IVEND]. */
...
/*                GFFOVE uses UDBAIL only when BAIL (see above) is set */
/*                to .TRUE., indicating that interrupt handling is */
/*                enabled. When interrupt handling is enabled, GFFOVE */
...
/*                function must still be passed to GFFOVE as */
...
/*                will be discarded before GFFOVE conducts its */
/*                search. */
...
/*         value for the target type (ephemeris object or ray), the */
...
/*         the FOV is any other shape, the same error check will be */
...
/*          .              GFSTEP,  GFREFN,  RPT,     GFREPI,  GFREPU, */
/*          .              GFREPF,  BAIL,    GFBAIL,  CNFINE,  RESULT ) */
...
/*        final output (the progress report is overwritten when it is */
/*        updated, so only the final update is captured here) was: */
...
/*     C */
/*           CALL GFFOVE ( INST,    TSHAPE,  RAYDIR, */
/*          .              TARGET,  RFRAME,  ABCORR,  OBSRVR,  TOL, */
/*          .              GFSTEP,  GFREFN,  RPT,     GFREPI,  GFREPU, */
/*          .              GFREPF,  BAIL,    GFBAIL,  CNFINE,  RESULT ) */
...
/*     The kernel files to be used by GFFOVE must be loaded (normally via */
/*     the SPICELIB routine FURNSH) before GFFOVE is called. */
