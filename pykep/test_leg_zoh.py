...
class leg_zoh_test(_ut.TestCase):
    def test_zoh_mc_ballistic(self):
        ...
        t0 = 10000  # MJD2000
        t1 = 10400  # MJD2000
        pl0 = _pk.planet(_pk.udpla.jpl_lp("Earth"))
        pl1 = _pk.planet(_pk.udpla.jpl_lp("Mars"))
        r0, _ = pl0.eph(t0)
        r1, _ = pl1.eph(t1)
        l = _pk.lambert_problem(r0=r0, r1=r1, tof=(t1 - t0) * _pk.DAY2SEC, mu=_pk.MU_SUN)
        m0 = 1000
        m1 = 1000
        ...
        for i in range(n_trials):
            ...
            veff = np.random.uniform(2000, 7000) * _pk.G0
            ...
            veff_nd = veff / V
            tgrid = np.linspace(
                t0 * _pk.DAY2SEC / TIME, t1 * _pk.DAY2SEC / TIME, nseg + 1
            )
    ...
    def test_zoh_mc_thrust(self):
        ...
        t0 = 1234  # MJD2000
        t1 = 3456  # MJD2000
        pl0 = _pk.planet(_pk.udpla.jpl_lp("Venus"))
        pl1 = _pk.planet(_pk.udpla.jpl_lp("Earth"))
        r0, _ = pl0.eph(t0)
        r1, _ = pl1.eph(t1)
        l = _pk.lambert_problem(r0=r0, r1=r1, tof=(t1 - t0) * _pk.DAY2SEC, mu=_pk.MU_SUN)
        v0 = l.v0[0]
    ...
    def test_compute_mc_grad(self):
        ...
        for i in range(100):
            veff = np.random.uniform(4000, 8000) * _pk.G0
            veff_nd = veff / V
            ta.pars[4] = 1. / veff_nd
            ta_var.pars[4] = 1. / veff_nd
            nseg = int(np.random.uniform(4, 20))
            tgrid = np.linspace(t0*_pk.DAY2SEC/TIME, t1*_pk.DAY2SEC/TIME, nseg+1)
            ...
            leg = _pk.leg.zoh(state0, controls.tolist(), state1, tgrid, cut = cut, tas = [ta, ta_var])
