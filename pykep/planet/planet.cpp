...
template <class Planet>
static inline class_<Planet, bases<planet::base>, bases<planet::keplerian>> planet_kep_wrapper(const char *name,
                                                                                               const char *descr)
{
...
BOOST_PYTHON_MODULE(_planet)
{
    ...
    class_<planet::python_base, boost::noncopyable>(
        "_base", "The base class for all planets, it cannot be instantiated as it contains pure virtual methods",
        init<optional<double, double, double, double, const std::string &>>(
    ...
    planet_wrapper<planet::keplerian>(
        "keplerian", "A planet with Keplerian ephemerides, derives from :py:class:`pykep.planet._base`")
        .def(init<optional<const epoch &, const array6D &, double, double, double, double, const std::string &>>())
        .def(init<const epoch &, const array3D &, const array3D &, double, double, double, double,
    ...
    planet_wrapper<planet::jpl_lp>("jpl_lp", "A solar system planet that uses the JPL low-precision ephemerides, "
                                             "derives from :py:class:`pykep.planet._base`")
    ...
    planet_wrapper<planet::tle>(
        "tle", "An Earth satellite defined from the TLE format, derives from :py:class:`pykep.planet._base`")
        ...
        .add_property("ref_mjd2000", &planet::tle::get_ref_mjd2000,
    ...
    planet_kep_wrapper<planet::mpcorb>(
        "mpcorb", "A planet from the MPCORB database, derives from :py:class:`pykep.planet.keplerian`")
        .def(init<optional<const std::string &>>("pykep.planet.mpcorb(line)\n\n"
                                                 "- line: a line from the MPCORB database file\n\n"
                                                 "Example::\n\n"
                                                 "  apophis = planet.mpcorb('99942   19.2   0.15 K107N 202.49545  "
                                                 "126.41859  204.43202    3.33173  0.1911104  1.11267324   0.9223398  "
                                                 "1 MPO164109  1397   2 2004-2008 0.40 M-v 3Eh MPCAPO     C802  "
                                                 "(99942) Apophis            20080109')"))
        .add_property("H", &kep_toolbox::planet::mpcorb::get_H,
                      "The asteroid absolute magnitude. This is assuming an albedo of 0.25 and using the formula at "
        ...
        .add_property("year_of_discovery", &kep_toolbox::planet::mpcorb::get_year_of_discovery,
                      "The year the asteroid was first discovered. In case the asteroid has been observed only once "
                      "(n_observations), this number is, instead, the Arc Length in days\n"
                      "Example::\n\n"
                      "  R = apophis.year_of_discovery");

    planet_kep_wrapper<planet::gtoc2>("gtoc2",
                                      "An asteroid from gtoc2, derives from :py:class:`pykep.planet.keplerian`")
        .def(init<optional<int>>("pykep.planet.gtoc2(ast_id)\n\n"
    ...
    planet_kep_wrapper<planet::gtoc5>("gtoc5",
                                      "An asteroid from gtoc5, derives from :py:class:`pykep.planet.keplerian`")
        .def(init<optional<int>>("pykep.planet.gtoc5(ast_id)\n\n"
                                 "- ast_id: a consecutive id from 1 to 7076 (Earth). The order is that of the original"
                                 "data file distributed by the Russian, see the (`gtoc portal "
                                 "<http://sophia.estec.esa.int/gtoc_portal/>`_). Earth is 7076\n\n"
                                 "Example::\n\n"
                                 "  russian_ast = planet.gtoc5(1)"));

    planet_kep_wrapper<planet::gtoc6>("gtoc6",
                                      "A Jupiter moon from gtoc6, derives from :py:class:`pykep.planet.keplerian`")
        .def(init<optional<const std::string &>>(
    ...
    planet_kep_wrapper<planet::gtoc7>("gtoc7",
                                      "An asteroid from gtoc7, derives from :py:class:`pykep.planet.keplerian`")
        .def(init<optional<int>>("pykep.planet.gtoc7(ast_id)\n\n"
                                 "- ast_id: a consecutive id from 0 (Earth) to 16256. The order is that of the "
                                 "original file , see the (`gtoc portal "
                                 "<http://sophia.estec.esa.int/gtoc_portal/>`_).\n\n"
                                 "Example::\n\n"
