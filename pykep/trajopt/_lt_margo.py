...
class lt_margo:
    ...
    def __init__(self,
        ...
        tof = [200, 500], m0 = 50.0, Tmax = 0.001, Isp = 2000.0, earth_gravity = False, sep = False, start = "earth")
        ...
        - earth_gravity: boolean specifying whether to include the Earth's gravity
        - sep: boolean specifying whether to use a NEP model (if ``False``) or a SEP model with Tmax dependent on the distance from the Sun (if ``True``)
    ...
    def plot_trajectory(self, x, units=pk.AU, plot_segments=True, plot_thrusts=False, ax=None):
        """
        ax = prob.plot_trajectory(self, x, units=AU, plot_segments=True, plot_thrusts=False, ax=None)

        - x: encoded trajectory
        - units: the length unit to be used in the plot
        - plot_segments: when true plots also the segments boundaries
        - plot_thrusts: when true plots also the thrust vectors
        ...
        pk.orbit_plots.plot_planet(self.__earth, pk.epoch(t0), units=units, legend=True, color=(0.7, 0.7, 1), ax=axis)
        pk.orbit_plots.plot_planet(self.target, pk.epoch(t0 + T), units=units, legend=True, color=(0.7, 0.7, 1), ax=axis)
        ...
        for i in range(fwd_seg):
            ...
            if self.__earth_gravity:
                r3 = sum([r**2 for r in dfwd[i]])**(3/2)
                disturbance = [mfwd[i] * pk.MU_EARTH / r3 * ri for ri in dfwd[i]]
                pk.orbit_plots.plot_taylor_disturbance(rfwd[i], vfwd[i], mfwd[i], ufwd[i], disturbance, fwd_dt[i], pk.MU_SUN, veff, N=10, units=units, color=(alphas[i], 0, 1-alphas[i]), ax=axis)
            else:
                pk.orbit_plots.plot_taylor(rfwd[i], vfwd[i], mfwd[i], ufwd[i], fwd_dt[i], pk.MU_SUN, veff, N=10, units=units, color=(alphas[i], 0, 1-alphas[i]), ax=axis)
            xfwd[i+1] = rfwd[i+1][0] / units
            yfwd[i+1] = rfwd[i+1][1] / units
            zfwd[i+1] = rfwd[i+1][2] / units
        if plot_segments:
            axis.scatter(xfwd[:-1], yfwd[:-1], zfwd[:-1], label='nodes', marker='o', s=1, c='b')
        ...
        for i in range(bwd_seg):
            ...
            if self.__earth_gravity:
                r3 = sum([r**2 for r in dbwd[-i-1]])**(3/2)
                disturbance = [mfwd[i] * pk.MU_EARTH / r3 * ri for ri in dbwd[-i-1]]
                pk.orbit_plots.plot_taylor_disturbance(rbwd[-i-1], vbwd[-i-1], mbwd[-i-1], ubwd[-i-1], disturbance, -bwd_dt[-i-1], pk.MU_SUN, veff, N=10, units=units, color=(alphas[-i-1], 0, 1-alphas[-i-1]), ax=axis)
            else:
                pk.orbit_plots.plot_taylor(rbwd[-i-1], vbwd[-i-1], mbwd[-i-1], ubwd[-i-1], -bwd_dt[-i-1], pk.MU_SUN, veff, N=10, units=units, color=(alphas[-i-1], 0, 1-alphas[-i-1]), ax=axis)
            xbwd[-i-2] = rbwd[-i-2][0] / units
            ybwd[-i-2] = rbwd[-i-2][1] / units
            zbwd[-i-2] = rbwd[-i-2][2] / units
        if plot_segments:
            axis.scatter(xbwd[1:], ybwd[1:], zbwd[1:], marker='o', s=1, c='b')
        ...
        if plot_thrusts:
            ...
            for (x,y,z,t) in zip(xfwd[:-1] + xbwd[:-1], yfwd[:-1] + ybwd[:-1], zfwd[:-1] + zbwd[:-1], throttles):
                axis.plot([x, x + t[0]],[y, y + t[1]],[z, z + t[2]], c = 'g')
    ...
    def plot_dists_thrust(self, x, ax=None):
        ...
        if self.__earth_gravity:
            axis.axhline(y = np.sqrt(pk.MU_EARTH * self.__sc.mass / (self.__sc.thrust * 0.1)) / pk.AU, c = 'g', ls = ":", lw = 1, label = "earth_g = 0.1*Tmax")
        # Plot thrust profile
        axis = axis.twinx()
        if self.__sep:
            max_thrust = self.__sc.thrust
            ...
            axis.step(np.concatenate((fwd_grid, bwd_grid[1:])), Tmaxs, where = "post", c = 'lightgray', linestyle = ':')
        else:
        ...
        axis.set_ylabel("T/Tmax$_{1AU}$",color='r')
        axis.tick_params('y',colors='r')
        axis.set_xlim([times[0],times[-1]])
        axis.set_ylim([0,max(thrusts) + 0.2])
