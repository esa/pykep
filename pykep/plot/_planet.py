...
def add_planet(ax, pla: pk.planet, when, units=pk.AU, **kwargs):
...
def add_planets(ax, plas: pk.planet, when, units=pk.AU, **kwargs):
...
def add_solar_system(
    ax, bodies=[1, 2, 3, 4, 5, 6], when=pk.epoch(0), s=[15, 2, 3, 3, 2, 8, 8, 5, 5]
):
    ...
    """
    # Sun
    pk.plot.add_sun(ax, s=s[0])

    # Mercury
    if 1 in bodies:
        udpla = pk.udpla.jpl_lp(body="Mercury")
        mercury = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(ax, mercury, c="tomato", label="")
        pk.plot.add_planet(ax, mercury, when=when, c="tomato", s=s[1], label = "Mercury")

    # Venus
    if 2 in bodies:
        udpla = pk.udpla.jpl_lp(body="Venus")
        venus = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(ax, venus, c="forestgreen", label="")
        pk.plot.add_planet(ax, venus, when=when, c="forestgreen", s=s[2], label = "Venus")

    # Earth
    if 3 in bodies:
        udpla = pk.udpla.jpl_lp(body="Earth")
        earth = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(ax, earth, c="royalblue", label="")
        pk.plot.add_planet(ax, earth, when=when, c="royalblue", s=s[3], label = "Earth")

    # Mars
    if 4 in bodies:
        udpla = pk.udpla.jpl_lp(body="Mars")
        mars = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(ax, mars, c="indianred", label="")
        pk.plot.add_planet(ax, mars, when=when, c="indianred", s=s[4], label = "Mars")

    # Jupiter
    if 5 in bodies:
        udpla = pk.udpla.jpl_lp(body="Jupiter")
        jupiter = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(ax, jupiter, c="tan", label="")
        pk.plot.add_planet(ax, jupiter, when=when, c="tan", s=s[5], label = "Jupiter")

    # Saturn
    if 6 in bodies:
        udpla = pk.udpla.jpl_lp(body="Saturn")
        saturn = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(ax, saturn, c="darkcyan", label="")
        pk.plot.add_planet(ax, saturn, when=when, c="darkcyan", s=s[6], label = "Saturn")
    ...
    # Uranus
    if 7 in bodies:
        udpla = pk.udpla.jpl_lp(body="uranus")
        uranus = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(
            ax,
            uranus,
            c="slateblue",
            label="",
        ...
        pk.plot.add_planet(ax, uranus, when=when, c="slateblue", s=s[7], label = "Uranus")

    # Neptune
    if 8 in bodies:
        udpla = pk.udpla.jpl_lp(body="neptune")
        neptune = pk.planet_to_keplerian(pk.planet(udpla), when=when)
        pk.plot.add_planet_orbit(
            ax,
            neptune,
            c="orchid",
        ...
        pk.plot.add_planet(ax, neptune, when=ep, c="orchid", s=s[7], label = "Neptune")
